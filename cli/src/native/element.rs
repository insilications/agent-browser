...
/// Runtime execution context for a document in its effective renderer session.
/// Internal frame-scoping operations select an isolated world; user-authored
/// JavaScript selects the document's default page world. An omitted context
/// address selects the target's default context for top-level operations.
#[derive(Debug, Clone)]
...
struct ContextEvaluateParams<'a> {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    unique_context_id: Option<&'a str>,
}
...
/// Resolve the selected frame's default page world. Unlike
/// `frame_execution_context`, this is for JavaScript supplied by the user and
/// exposes globals created by the document's own scripts.
pub(super) fn main_world_execution_context(
    client: &CdpClient,
    top_session_id: &str,
    frame: Option<&FrameContext>,
) -> Result<FrameExecutionContext, String> {
    ...
    let context = client
        .default_execution_context(&frame.session_id, &frame.frame_id)
        .ok_or_else(|| {
            format!(
                "Could not find the page execution context for frame {}",
                frame.frame_id
            )
        })?;
    let has_unique_id = context.unique_context_id.is_some();
    Ok(FrameExecutionContext {
        session_id: frame.session_id.clone(),
        context_id: (!has_unique_id).then_some(context.context_id),
        unique_context_id: context.unique_context_id,
    })
}
    ...
    client
        .send_command_typed(
            ...
            &ContextEvaluateParams {
                ...
                unique_context_id: context.unique_context_id.as_deref(),
            },
...
/// Prefer the recorded renderer session, then a dedicated OOPIF session, then
/// the top-level session. An OOPIF root needs no frameId; a same-process child
/// needs its frameId within its containing renderer session, regardless of origin.
...
/// Prefer the session recorded with the ref's backend node. Older refs without
/// an explicit session fall back to their frame's dedicated OOPIF session or
...
mod tests {
    ...
    #[test]
    fn context_evaluate_params_select_one_context_address() {
        let unique = serde_json::to_value(ContextEvaluateParams {
            expression: "window.ready",
            return_by_value: true,
            await_promise: true,
            context_id: None,
            unique_context_id: Some("unique-context"),
        })
        .unwrap();
        assert_eq!(unique["uniqueContextId"], "unique-context");
        assert!(unique.get("contextId").is_none());

        let numeric = serde_json::to_value(ContextEvaluateParams {
            expression: "window.ready",
            return_by_value: true,
            await_promise: true,
            context_id: Some(7),
        ...
        .unwrap();
        assert_eq!(numeric["contextId"], 7);
        assert!(numeric.get("uniqueContextId").is_none());
    }
