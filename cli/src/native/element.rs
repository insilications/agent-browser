...
static ACTIVE_FRAME: std::sync::OnceLock<std::sync::Mutex<Option<SelectedFrame>>> =
    std::sync::OnceLock::new();

pub fn set_active_frame(frame: Option<&SelectedFrame>) {
    *ACTIVE_FRAME
...
fn active_frame() -> Result<Option<FrameContext>, String> {
    match ACTIVE_FRAME.get().and_then(|m| m.lock().unwrap().clone()) {
        Some(SelectedFrame::Available(context)) => Ok(Some(context)),
        Some(SelectedFrame::Unavailable { .. }) => Err(SELECTED_FRAME_UNAVAILABLE.to_string()),
        None => Ok(None),
    }
}
