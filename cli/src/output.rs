...
Lifecycle:
  If the selected frame is removed, frame-scoped commands fail instead of
  falling back to main. Use `frame main` to recover. Renderer swaps that keep
  the same frame ID are reconciled automatically. Top-document navigation and
  successful tab changes reset frame selection to main.
