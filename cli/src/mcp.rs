...
fn parity_tools() -> Vec<Value> {
    vec![
        ...
        tool(
            TOOL_FRAME_SWITCH,
            "Frame switch",
            "Switch to an iframe by selector or snapshot ref, resolved relative to the current frame. Removed selections fail explicitly until frame main or top-document navigation resets them.",
        ...
        tool(
            TOOL_FRAME_MAIN,
            "Frame main",
            "Switch to the main frame, including recovery after a selected iframe is removed.",
            json!({}),
            &[],
        ),
        tool(
