...
fn parity_tools() -> Vec<Value> {
    vec![
        ...
        tool(
            TOOL_FRAME_SWITCH,
            "Frame switch",
            "Switch to an iframe by selector or snapshot ref, resolved relative to the current frame. The same frame ID follows renderer changes before commands. Removed selections return frame_gone; unresolved renderer preparation returns frame_not_ready. Use frame main to recover, or select through a valid surviving iframe ref.",
