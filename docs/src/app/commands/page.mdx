...
The `frame` command accepts element refs (`@e3` or `e3`) and CSS selectors (`"#my-iframe"`). Selection is relative to the current frame, so after entering one iframe the next selector is evaluated in that iframe's document. A ref from a new scoped snapshot is resolved in the CDP session that produced it, including for nested same-process, cross-origin, and out-of-process iframes. Each snapshot replaces the ref map, so refs from older snapshots intentionally become invalid.

`eval` and `wait --fn` run in the current frame's page world, so application globals defined by that document are visible. The internal machinery used to scope operations to a selected frame continues to use an isolated world.

Selection is preserved across short renderer detach/attach transitions when the frame ID stays stable. If the selected frame is removed, frame-scoped commands report `Selected frame is no longer available` instead of falling back to the main document. Run `agent-browser frame main`, then select the desired frame again. Explicit top-document navigation (`open`, `back`, `forward`, `reload`, and `set content`) and successful tab changes reset frame selection to main.
...
Console messages and uncaught exceptions are collected across the active tab, including its out-of-process iframes. OOPIFs belonging to background tabs remain excluded.
