...
`snapshot --delta` returns `full`, `unchanged`, or incremental updates per tab and option set. Selected-frame, document, renderer, URL, or option changes and large deltas return full state. There is one baseline per tab, so returning to a previously selected frame also returns full state; `--full` resets the baseline. Apply a delta’s `changes` to ref metadata and `treeChange` to the previous tree: split on newlines, replace `deleteCount` lines at zero-based `startLine` with `lines`, then rejoin. Apply both to `baseRevision` before advancing to `revision`.
...
The `frame` command accepts element refs (`@e3` or `e3`) and CSS selectors (`"#my-iframe"`). Selection is relative to the current frame, so after entering one iframe the next selector is evaluated in that iframe's document. A ref from a new scoped snapshot is resolved in the CDP session that produced it, including for nested same-process, cross-origin, and out-of-process iframes. Surviving DOM elements retain their refs within the same document and renderer. Replaced documents invalidate affected refs without recycling their IDs; take a fresh snapshot after navigation.
