...
The `frame` command accepts element refs (`@e3` or `e3`) and CSS selectors (`"#my-iframe"`). Selection is relative to the current frame, so after entering one iframe the next selector is evaluated in that iframe's document. A ref from a new scoped snapshot is resolved in the CDP session that produced it, including for nested same-process, cross-origin, and out-of-process iframes. Each snapshot replaces the ref map and can reuse reference numbers for different elements. Use the refs from the latest snapshot.
