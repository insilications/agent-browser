...
Frame switching is relative. After entering an iframe, a selector or a ref from the new scoped snapshot resolves inside the selected frame, including when nesting crosses an out-of-process iframe boundary. Surviving DOM elements retain refs in the same document; replacing a document invalidates its refs without recycling IDs.
