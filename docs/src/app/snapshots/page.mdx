...
If a selected frame is removed, scoped snapshots and other frame-scoped commands fail explicitly rather than falling back to the main document. Use `agent-browser frame main` to recover. Renderer swaps that preserve the frame ID are reconciled automatically, while explicit top-document navigation and successful tab changes reset selection to main.
